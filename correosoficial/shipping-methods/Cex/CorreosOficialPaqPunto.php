...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_29');

function request_shipping_quote_method_29() {
	if (!class_exists('WC_Paq_Punto')) {

		class WC_Paq_Punto extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_29');

function add_request_shipping_quote_29( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Punto::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Punto';
	}
