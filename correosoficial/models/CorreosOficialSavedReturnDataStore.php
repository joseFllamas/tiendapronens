...
class CorreosOficialSavedReturnDataStore extends WC_Data_Store_WP {
	...
	public function __construct() {
		global $wpdb;
		$this->wpdb  = $wpdb;
		$this->table = $wpdb->prefix . 'correos_oficial_saved_returns';
	}
	...
	public function get_all( $id_order ) {
		...
		if ($results) {
			...
			foreach ($results as $row) {
				$order = new CorreosOficialSavedReturn();
				$order->set_props($row);
				$orders[] = $order;
			}
			return $orders;
