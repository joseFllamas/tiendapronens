...
class Analitica {
	...
	public function __construct() {
		...
		$this->host = CorreosOficialEnvironment::getAnaliticaHost();
	...
	public function moduleRecord() {
		...
		$body = array(
			'shopDistinctive' => $this->shop_url,
			'moduleCode' => CorreosOficialMarketplace::getModuleCode(),
		...
		foreach ($senders as $sender) {
			...
			if ($sender['correos_code']) {
				...
				if (!empty($correosCode)) {
					...
					if (!empty($correosCode['CorreosClientID']) && $correosCode['CorreosClientID'] != 'n/a') {
						$correosId = $correosCode['CorreosClientID'];
					}

					if(!empty($correosCode['CorreosUser']) && $correosCode['CorreosUser'] != 'n/a'){
                        $accounts[0]['userWebservice']  = $correosCode['CorreosUser'];
                    }

                    if(!empty($correosCode['CorreosOv2Code']) && $correosCode['CorreosOv2Code'] != 'n/a'){
                        $accounts[0]['userOv2']  = $correosCode['CorreosOv2Code'];
                    }
				}
			}
			...
			$products = CorreosOficialProduct::get_all_products();
			$listed_products = [];

			foreach ( $products as $product) {
				$listed_products[] =  [
					'sapCode' => $product['codigoProducto'],
					'productName' => $product['name'],
					'activeInModule' => $product['active'] == 1 ? true : false,
				];
			}
