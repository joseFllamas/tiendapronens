...
# -*- coding: utf-8 -*-
"""
Genera con Nano Banana una foto de cada bolsa impermeable del CSV CERRADA
(con la cuerda superior estirada y fruncida) y con aspecto de llevar algo
dentro (desayuno u objetos de guarderia, sin que se vean), sobre fondo gris
#f1f1f0 y con el mismo margen que el resto del catalogo. Lienzo 3:4 (1200x1600).

Lee el CSV de productos (producto_id, titulo, ruta, ...) y usa la foto del
producto como referencia para reproducir la bolsa exacta.

Requisitos:  pip install google-genai pillow numpy scipy
             export GEMINI_API_KEY="tu_clave"

Uso (desde la raiz del proyecto):
  python3 scripts/unificacion-imagenes/generar_bolsas_impermeables.py
  # opcionales:
  ...
  #   --w 1600 --h 1200 --margin 150 --gray "#f1f1f0"
...
def build_prompt(_unused=None):
    return (
        "Using the children's drawstring nursery bag shown in this reference product photo, "
        "create a photorealistic studio product photograph of EXACTLY this same bag, now CLOSED: "
        "the top drawstring pulled tight so the fabric gathers into a neat closed top. The bag "
        "stands upright, gently filled and softly rounded, as if it contains a child's breakfast "
        "and small nursery items inside; the items themselves must NOT be visible. "

        "CORD: the drawstring is visible ONLY as the gathered closure at the very top of the bag. "
        "Do not show cords hanging down the sides or across the front; any remaining cord falls "
        "behind the bag and is not visible. No loose, tangled or dangling strings. "

        "NO INVENTED HARDWARE: this is a simple flat printed fabric bag with a plain fabric "
        "drawstring at the top hem and nothing else. Do NOT add black corner patches, reinforced "
        "or triangular corners, eyelets, grommets, rivets, metal rings, clips, buckles, zippers, "
        "shoulder straps or backpack straps. The bottom corners must be plain printed fabric, "
        "exactly like the reference. "

        "PRINT FIDELITY: reproduce the printed artwork exactly, with the same colors, characters, "
        "text and logo. Keep the artwork undistorted: the folds must be soft, few, and placed away "
        "from the important central artwork. NEVER wrinkle, warp, squash or distort faces, eyes or "
        "other key details of the printed characters; they must stay clean, sharp and perfectly "
        "readable, like in a professional studio catalogue photo where the folds are deliberately "
        "arranged to flatter the product. "

        "FRAMING: vertical 3:4 portrait, the bag centered and filling most of the frame, with a "
        "small even margin of plain background all around. "

        "BACKGROUND: plain flat light-gray #f1f1f0 covering the ENTIRE image edge to edge. Soft "
        "even studio lighting, no props, no text or watermarks. Completely shadowless product "
        "photography: absolutely no shadow, no contact shadow and no reflection anywhere."
    )
...
def pad_frame_img(im, cfg):
    """Quita sombra y restos de fondo (neutros no mas claros que el fondo),
    remapea al gris exacto y centra el producto con margen. Conserva cuerdas
    y detalles mas claros que el fondo."""
    import numpy as np
    ...
    lum = a.mean(2); bglum = bgc.mean()
    chroma = a.max(2) - a.min(2)
    target = np.array(cfg["gray"], dtype=np.float32)
    # fondo/sombra: neutro y no mucho mas claro que el fondo (cubre gradientes)
    neutro = (chroma < 20) & (lum < bglum + 8) & (lum > bglum * 0.5) & (dist < 140)
    core = ndimage.binary_fill_holes((dist > 45) & ~neutro)
    flatten = neutro & ~core
    m = np.clip(ndimage.gaussian_filter(flatten.astype(np.float32), sigma=2.0), 0, 1)
    m[core] = 0.0
    a = a * (1 - m[:, :, None]) + target[None, None, :] * m[:, :, None]
    peso = np.clip(1.0 - dist / (2.0 * cfg["tol"]), 0.0, 1.0)
    peso[flatten] = 0.0
    a = np.clip(a + (target - bgc)[None, None, :] * peso[:, :, None], 0, 255)
    im2 = _Image.fromarray(a.astype("uint8"))
    # caja: producto + detalles claramente mas claros que el fondo (cuerdas)
    claros = (dist > 15) & (lum > bglum + 10) & ~flatten
    caja = core | claros
    ys, xs = np.where(caja)
    if len(xs) < 30:
        return None
...
def pad_simple(im, cfg):
    """Plan B sin scipy: remapeo suave del tono de fondo + encuadre."""
    import numpy as np
    from PIL import Image as _Image
    rgb = im.convert("RGB")
    a = np.asarray(rgb).astype(np.float32)
...
def main():
    ...
    ap.add_argument("--csv", default="salida_unificacion/bolsasimpermeables.csv")
    ap.add_argument("--webroot", default="web")
    ap.add_argument("--out", default="salida_unificacion/bolsasimpermeables_cerradas")
